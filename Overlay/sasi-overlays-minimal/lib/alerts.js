...
(function() {
  ...
  function showPopup(event) {
    ...
    if ((event.type === 'tip' || event.type === 'cheer') && !event.message && (event.stickerUrl || event.isSticker)) {
      ...
      // Bug fix (sasi-studio v2): trigger SASI_NOTIFY for recognized games so
      // the visual wheel/box/highlight animation in just-chatting.html fires
      // — not just the alert card. just-chatting listens for SASI_NOTIFY
      // storage events and runs the appropriate animation.
      const rewardKey = (event.rewardName || '').toLowerCase();
      const gameType = rewardKey === 'wheel of fortune' ? 'wheel'
                     : rewardKey === 'mystery box' ? 'mystery'
                     : rewardKey === 'highlight message' ? 'highlight' : null;
      if (gameType) {
        // (a) Add to SASI_NOTIFY queue (powers the dashboard's notifications panel + cross-window sync)
        if (window.SASI_NOTIFY) {
          window.SASI_NOTIFY.add({
            type: gameType,
            user: event.name,
            title: event.rewardName,
            body: event.message || '',
            duration: duration,
          });
        }
        // (b) Direct call to the visual-animation globals (only present in just-chatting.html).
        // Same-window storage events don't fire to the originator, so we can't rely on
        // (a) to trigger the animation if alerts.js + just-chatting share a window.
        if (typeof window.SPIN_WHEEL === 'function' && gameType === 'wheel') window.SPIN_WHEEL(event.name);
        else if (typeof window.OPEN_MYSTERY === 'function' && gameType === 'mystery') window.OPEN_MYSTERY(event.name);
        else if (typeof window.HIGHLIGHT_MSG === 'function' && gameType === 'highlight') window.HIGHLIGHT_MSG(event.name, event.message || '');
      }
